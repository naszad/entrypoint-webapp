...
const TardiesPage = () => {
  ...
  const [studentTardies, setStudentTardies] = useState<StudentTardyInfo[]>([]);
  ...
  const fetchStudentTardies = async (
    filters: FilterValue[], 
    sortField: string, 
    ...
    try {      
      ...
      const url = `/api/students/${params.id}/tardies-details?${queryParams.toString()}`;
      ...
      if (!response.ok) {
        setAlertMessage({ 
          type: 'destructive', 
          message: data.error || 'Failed to fetch student tardies'
        });
        return;
      }

      setStudentTardies(data.tardies);
      setTotalCount(data.count);
    } catch (err) {
      setAlertMessage({ 
        type: 'destructive', 
        message: `Failed to fetch student tardies ${err instanceof Error ? err.message : ''}`
      });
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    ...
    <div className="flex flex-col w-full h-full">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-2xl font-bold text-gray-700">Tardies</h3>
      </div>
