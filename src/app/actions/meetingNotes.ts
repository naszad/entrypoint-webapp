...
async function generateSummaryFromNotes(notes: string): Promise<string> {
  const result = streamText({
    ...
    messages: [
      {
        role: "user",
        content: `Generate a one-sentence summary from these meeting notes: ${notes}`,
      },
    ],
  });
...
// Helper function to save meeting note to database
async function saveMeetingNoteToDatabase(
  userId: string,
  studentId: string,
  summary: string,
  notes: string,
  ...
  const insertData = {
    ...
    ...(meetingDate && {
      created_at: new Date(meetingDate).toISOString(),
      updated_at: new Date(meetingDate).toISOString(),
    }),
  };
...
async function generateTagSuggestions(
  ...
  try {
    ...
    const [existingCustomerTags, existingStudentTags, allCategories] =
      await Promise.all([
        getAllTagsForCustomer(),
    ...
    const tagsByCategory = new Map<
      string,
      { tagId: string; name: string; categoryId: string; values: string[] }[]
    >();
...
${Array.from(tagsByCategory.entries())
  .map(
    ([category, tags]) => `${category}: ${tags.map((t) => t.name).join(", ")}`,
  )
  .join("\n")}
...
${Array.from(studentTagValues).join(", ")}`;
      ...
      tags: z
        .array(
          z.object({
            tagName: z
              .string()
              .describe("Exact name of existing tag or concise new tag name"),
            categoryName: z
              .string()
              .describe(
                "Category name from the EXISTING CATEGORIES list above",
              ),
            value: z.string().describe("Concise value for the tag"),
          }),
        )
        .max(5)
        .describe("Up to 5 suggested tags based on meeting notes"),
    });
    ...
    suggestedTags = suggestedTags.filter(
      (tag) => !studentTagValues.has(`${tag.name}:${tag.value}`),
    );
  } catch (tagError) {
...
export async function addMeetingNotes(
  ...
  try {
    ...
    // Generate AI summary from the provided notes
    const summary = await generateSummaryFromNotes(notes);

    // Get student email for response
    ...
    const savedNote = await saveMeetingNoteToDatabase(
      userId,
      studentId,
      summary,
      notes,
    ...
    return {
      meetingNoteId: savedNote.meeting_note_id,
      summary: summary,
      notes: notes,
      transcript: null, // No transcript for manual notes
...
// Helper function to generate meeting notes from transcript text
export async function generateMeetingNotesFromTranscript(
  ...
  try {
    const result = streamText({
      ...
      messages: [
        {
          role: "user",
          content: `Generate a one sentence summary and detailed meeting notes in bullet points from this transcript: ${transcriptText}
...
// Helper function to process audio and generate meeting notes
export async function generateMeetingNotesFromAudio(
  audioBlob: Blob,
  userId: string,
  studentId: string,
) {
  try {
    ...
    // Use the transcript-based function
    return generateMeetingNotesFromTranscript(
      transcript.text,
      userId,
      studentId,
    );
...
export async function generateMeetingNotesAction(
  audioBlob: Blob,
  userId: string,
  studentId: string,
) {
  return generateMeetingNotesFromAudio(audioBlob, userId, studentId);
}
