...
DROP VIEW IF EXISTS school_student_link_students_view;

create or replace view "public"."school_student_link_students_gpa_view" as  WITH school_configs AS (
         SELECT s_1.school_id,
           ...
           FROM (( SELECT DISTINCT school_student_link.school_id
                   FROM school_student_link) s_1
             LEFT JOIN LATERAL ( SELECT fetch_public_config.value
                   FROM fetch_public_config('final_grade_code'::text, NULL::uuid, s_1.school_id, NULL::uuid) fetch_public_config(config_id, config_key, value, customer_id, school_id, user_id, created_at, updated_at)
                 LIMIT 1) cfg ON (true))
        ), gpa_results AS (
         SELECT gpa_result.student_id,
           ...
           FROM (school_configs s_1
             JOIN LATERAL ( SELECT calculate_gpa_for_students.student_id,
                   ...
                   FROM calculate_gpa_for_students(( SELECT array_agg(school_student_link.student_id) AS array_agg
