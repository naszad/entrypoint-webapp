...
- Each column declaration maps to a `FilterValue` token (`{ key, condition, value }`) that travels through the table pipeline. Keep accessor keys stable—back-end services translate them into SQL column names with helper functions like `getColumnName` / `getGradeColumnName`. When the key needs suffixes (`From`, `To`, `RecentOnly`) those helpers strip them before querying.【F:src/components/DataTable/DataTable.tsx†L33-L55】【F:src/libs/studentsService.ts†L94-L214】
...
### Filter UI widgets
- `FilterDropdown` renders the correct control for the column: condition select for free-text/number filters, dropdown lists, remote multi-select checklists, or date pickers with "Last N days" shortcuts. "Recent only" values disable the range inputs so we never send conflicting filters. The dropdown also exposes column-level sort buttons.【F:src/components/DataTable/FilterDropdown.tsx†L10-L285】
...
|-------------|---------------|-------|
| Text / number | `columnId:condition:value` | Conditions come from `getFilterConditionsByType` (`contains`, `eq`, `gt`, etc.).【F:src/components/DataTable/FilterDropdown.tsx†L108-L124】【F:src/utils/filterConditions.ts†L1-L35】 |
...
| GPA special case | `gpa:<operator>:<value>` | `applyFilters` skips it; GPA filters run after Supabase returns results so totals stay accurate.【F:src/libs/studentsService.ts†L141-L215】【F:src/libs/studentsService.ts†L460-L510】 |
